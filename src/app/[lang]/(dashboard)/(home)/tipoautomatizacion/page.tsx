...
const TipoAutomatizacionAgregarDialog = dynamic(
  () => import("./components/TipoAutomatizacionAgregar").then(mod => ({
    default: ({ onTipoAutomatizacionCreado }: { onTipoAutomatizacionCreado: () => void }) => (
      <DialogContent overflowVisible={true} size="3xl">
        <DialogHeader>
          <DialogTitle>Agregar Tipo de Automatización</DialogTitle>
          <DialogDescription>
            Crear un nuevo tipo de automatización en el sistema
          </DialogDescription>
        </DialogHeader>
        <mod.TipoAutomatizacionAgregar onTipoAutomatizacionCreado={onTipoAutomatizacionCreado} />
      </DialogContent>
    )
  })),
  {
...
const DataTablePage = () => {
  ...
  const { refreshTrigger } = useRefreshTableTipoAutomatizacion();
  
  const { hasSpecificRoute } = useModulePermissions("tipoAutomatizacion");
  const hasCreatePermission = hasSpecificRoute("POST", "tipoAutomatizacion/crear");

  const handleTipoAutomatizacionCreado = () => {
    setDialogOpen(false);
  };
    ...
    <div className="space-y-5">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>Tipo Automatización</CardTitle>
          {hasCreatePermission && (
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <Suspense fallback={
                ...
                <TipoAutomatizacionAgregarDialog onTipoAutomatizacionCreado={handleTipoAutomatizacionCreado} />
              </Suspense>
            </Dialog>
          )}
        </CardHeader>
          ...
          <Suspense fallback={
            <div className="p-8 text-center">
              <div className="inline-flex items-center justify-center">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium text-gray-700">Cargando tipos de automatización</p>
