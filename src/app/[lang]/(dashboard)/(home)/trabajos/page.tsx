...
const TrabajoAgregarDialog = dynamic(
  () => import("./components/TrabajoAgregar").then(mod => ({
    default: ({ onTrabajoCreado }: { onTrabajoCreado: () => void }) => (
      <DialogContent overflowVisible={true} size="3xl">
        <DialogHeader>
          <DialogTitle>Crear Nuevo Trabajo</DialogTitle>
          <DialogDescription>
            Registre un nuevo trabajo asignando técnicos, fechas y detalles
          </DialogDescription>
        </DialogHeader>
        <mod.TrabajoAgregar onTrabajoCreado={onTrabajoCreado} />
      </DialogContent>
    )
  })),
  {
...
const TrabajosPage = () => {
  ...
  const { refreshTrigger } = useRefreshTableTrabajos();
  const { hasSpecificRoute } = useModulePermissions("trabajos");
  
  const hasCreatePermission = hasSpecificRoute("POST", "trabajos/crear");
          ...
          {hasCreatePermission && (
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <Suspense fallback={
                ...
                <TrabajoAgregarDialog onTrabajoCreado={handleTrabajoCreado} />
              </Suspense>
            </Dialog>
          )}
        </CardHeader>
          ...
          <Suspense fallback={
            <div className="p-8 text-center">
              <div className="inline-flex items-center justify-center">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium text-gray-700">Cargando trabajos</p>
