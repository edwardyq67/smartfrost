...
import { sensoresService, CreateSensorData } from "@/lib/sensores/UseSensores";
...
export function SensorCrear({ onSensorCreado }: { onSensorCreado?: () => void }) {
  ...
  const crearSensor = async (data: SensorFormData) => {
    try {
      ...
      const sensorData: CreateSensorData = {
        ...
        funcion: parseInt(data.funcion) || 0,
        offset: data.offset.trim(),
        imei: data.imei.trim(),
        identificador: data.identificador.trim(),
        maximo: maximo,
        minimo: minimo,
        control: parseInt(data.control)
      };
