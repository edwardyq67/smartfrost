...
const MapaAgregarDialog = dynamic(
  () => import("./components/MapaAgregar").then(mod => ({
    default: ({ onMapaCreado }: { onMapaCreado: () => void }) => (
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Agregar Mapa</DialogTitle>
          <DialogDescription>
            Crear un nuevo mapa en el sistema
          </DialogDescription>
        </DialogHeader>
        <mod.MapaAgregar onMapaCreado={onMapaCreado} />
      </DialogContent>
    )
  })),
  {
...
const DataTablePage = () => {
  ...
  const { refreshTrigger } = useRefreshTableMapa();
  const { hasSpecificRoute } = useModulePermissions("mapa");
  
  const hasCreatePermission = hasSpecificRoute("POST", "mapa/crear");

  const handleMapaCreado = () => {
    setDialogOpen(false);
  };
          ...
          {hasCreatePermission && (
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <Suspense fallback={
                ...
                <MapaAgregarDialog onMapaCreado={handleMapaCreado} />
              </Suspense>
            </Dialog>
          )}
        </CardHeader>
          ...
          <Suspense fallback={
            <div className="p-8 text-center">
              <div className="inline-flex items-center justify-center">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium text-gray-700">Cargando lista de mapas</p>
